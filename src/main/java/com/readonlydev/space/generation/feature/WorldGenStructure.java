package com.readonlydev.space.generation.feature;
...
import com.readonlydev.space.generation.util.GenRandom;
import com.readonlydev.space.generation.util.GenUtility;
import com.readonlydev.space.generation.util.IStructure;
...
public class WorldGenStructure extends WorldGenerator implements IStructure {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		this.generateStructure(worldIn, position, GenRandom.choice(Rotation.values()));
		return true;
	}
	...
	private Template getTemplate(World world) {
		...
		ResourceLocation location = new ResourceLocation(ExoInfo.MODID, structureName);
	...
	public BlockPos getCenter(World world) {
		...
		return new BlockPos(Math.floorDiv(template.getSize().getX(), 2), Math.floorDiv(template.getSize().getY(), 2),
				Math.floorDiv(template.getSize().getZ(), 2));
	}
	...
	public int getYGenHeight(World world, int x, int z) {
		...
		return GenUtility.getAverageGroundHeight(world, x, z, templateSize.getX(), templateSize.getZ(),
				this.getMaxVariation(world));
	}
	...
	public void generateStructure(World world, BlockPos pos, Rotation rotation) {
		if (getTemplate(world) != null) {
			...
			rotations.put(Rotation.CLOCKWISE_180,
