...
package com.readonlydev.space.yzceti.b.worldgen;
...
import com.readonlydev.space.yzceti.d.BiomeDecoratorOther;
...
public class YzCetiBBiomes extends ExoBiome {

	protected YzCetiBBiomes(BiomeData biomeData) {
		super(biomeData);
	}
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 == 0) {
				...
				if (iblockstate2.getMaterial() == Material.AIR) {
					j = -1;
				} else if (iblockstate2.getBlock() == YzCetiBlocks.B.YZB_SEDIMENTARYROCK) {
					if (j == -1) {
						if (k <= 0) {
							topState = AIR;
							fillState = STONE;
