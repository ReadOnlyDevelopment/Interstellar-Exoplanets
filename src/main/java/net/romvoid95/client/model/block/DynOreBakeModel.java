package net.romvoid95.client.model.block;
...
public class DynOreBakeModel implements IBakedModel {
	...
	protected IBakedModel getFallback () {
		if (fallback != null)
			return fallback;
		fallback = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getModelManager()
				.getMissingModel();
		return fallback;
	}
	...
	public DynOreBakeModel(IBakedModel oreModel) {
	...
	public List<BakedQuad> getQuads (@Nullable IBlockState state, @Nullable EnumFacing side, @Nonnull long rand) {
		if (Minecraft.getMinecraft().player != null) {
			...
			IBlockState      baseState = OreHelper
					.getDimensionOreBase(Minecraft.getMinecraft().player.world.provider.getDimensionType());
			String           cacheKey  = baseState.toString() + "," + (state != null ? state.toString() : "null") + ":"
					+ (side != null ? side.getName2() : "null") + ":" + (layer != null ? layer.toString() : "null");
			if (!cache.containsKey(cacheKey)) {
				ImmutableList.Builder<BakedQuad> builder = ImmutableList.builder();
				if (layer == null || layer == BlockRenderLayer.SOLID)
					builder.addAll(Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
							.getModelForState(baseState).getQuads(baseState, side, rand));
