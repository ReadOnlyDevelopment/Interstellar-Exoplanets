...
public class ExoRainRenderer extends IRenderHandler {
		public static final ExoRainRenderer INSTANCE = new ExoRainRenderer();

		private static final ResourceLocation RAIN_TEXTURES = Assets.getTexture("heavyrain");
			...
			if (rainStrength > 0.0F) {
				...
				for (int layerZ = pz - layers; layerZ <= pz + layers; ++layerZ) {
					for (int layerX = px - layers; layerX <= px + layers; ++layerX) {
						...
						double rainXCoord = this.rainXCoords[coordsIndex] * 0.5D;
						double rainYCoord = this.rainYCoords[coordsIndex] * 0.5D;
						...
						if (layerY != maxLayerY) {
							...
							double dx = layerX + 0.5F - entity.posX;
							double dz = layerZ + 0.5F - entity.posZ;
							float distance = MathHelper.sqrt(dx * dx + dz * dz) / layers;
							...
							builder.pos(layerX - rainXCoord + 0.5D, maxLayerY, layerZ - rainYCoord + 0.5D)
							.tex(0.0D, layerY * 0.25D + uvShiftY + uvShiftY).color(r, g, b, visibility)
							.lightmap(lightmapX, lightmapY).endVertex();
							builder.pos(layerX + rainXCoord + 0.5D, maxLayerY, layerZ + rainYCoord + 0.5D)
							.tex(1.0D, layerY * 0.25D + uvShiftY + uvShiftY).color(r, g, b, visibility)
							.lightmap(lightmapX, lightmapY).endVertex();
							builder.pos(layerX + rainXCoord + 0.5D, layerY, layerZ + rainYCoord + 0.5D)
							.tex(1.0D, maxLayerY * 0.25D + uvShiftY + uvShiftY).color(r, g, b, visibility)
							.lightmap(lightmapX, lightmapY).endVertex();
							builder.pos(layerX - rainXCoord + 0.5D, layerY, layerZ - rainYCoord + 0.5D)
							.tex(0.0D, maxLayerY * 0.25D + uvShiftY + uvShiftY).color(r, g, b, visibility)
							.lightmap(lightmapX, lightmapY).endVertex();
						}
					}
				}
