...
public class EntityUtil {
	private static EnumFacing[] facings = new EnumFacing[] {
	...
	static {
		try {
			...
			playersWatchingChunk = ReflectionHelper.changeFieldAccess(clazz, "players", "field_187283_c");
		}
		catch (ClassNotFoundException e) {
			ExoplanetsMod.logger.error("Failed to get PlayerChunkMap class.", e);
		}
	...
	*/
	...
	public static List<EntityPlayerMP> getPlayersWatchingChunk (WorldServer world, int x, int z) {
		...
		try {
			...
			ExoplanetsMod.logger.info("Failed to get players watching chunk :", e);
			return new ArrayList<>();
		}
	}
		...
		for (int i = 0; i < nb; ++i) {
			for (int j = 0; j < nb; ++j) {
				for (int k = 0; k < nb; ++k) {
					...
					ParticleDigging fx = (ParticleDigging) factory.createParticle(0, world, fxX, fxY, fxZ, fxX
							- pos.getX() - 0.5D, fxY - pos.getY() - 0.5D, fxZ - pos.getZ() - 0.5D, id);
	...
	*/
	...
	public static void addHitEffects (World world, RayTraceResult target, ParticleManager particleManager, IBlockState... states) {
		...
		ParticleDigging.Factory factory = new ParticleDigging.Factory();
		ParticleDigging         fx      = (ParticleDigging) factory
				.createParticle(0, world, fxX, fxY, fxZ, 0, 0, 0, id);
