...
public class ChunkProviderYzCetiC extends ChunkProviderBase {
	...
	private MapGenExoCaveGen        caveGenerator   = new MapGenExoCaveGen(YzCetiBlocks.YzCetiC.YZC_IGNEOUS, 0, 1, 2);
				...
				for (int x = 0; x < ChunkProviderYzCetiC.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderYzCetiC.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4
								.getValue(x * ChunkProviderYzCetiC.CHUNK_SIZE_X
										+ x, cz * ChunkProviderYzCetiC.CHUNK_SIZE_Z + z)
