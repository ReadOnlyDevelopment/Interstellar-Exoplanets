...
public class MapGenCetiCaves extends MapGenBaseMeta {
	@Override
	...
	protected void recursiveGenerate (World par1World, int xChunkCoord, int zChunkCoord, int origXChunkCoord, int origZChunkCoord, ChunkPrimer primer) {
		if (this.rand.nextInt(100) == 0) {
			...
			this.generateLargeCaveNode(this.rand
					.nextLong(), origXChunkCoord, origZChunkCoord, primer, xPos, yPos, zPos);
		}
	}
	...
	protected void generateLargeCaveNode (long par1, int origXChunkCoord, int origZChunkCoord, ChunkPrimer primer, double xPos, double yPos, double zPos) {
		this.generateCaveNode(par1, origXChunkCoord, origZChunkCoord, primer, xPos, yPos, zPos, 1.0F
				+ this.rand.nextFloat() * 6.0F, 10.0F, 10.0F, -1, -1, 0.2D);
	}
								...
								for (int var50 = caveMaxY - 1; var50 >= caveMinY; --var50) {
									...
									if (var59 * var59 + var51 * var51 + var46 * var46 < 1.0D) {
										if (var51 > -0.7D) {
											...
											if (block == YzCetiBlocks.YzCetiD.YZD_IGNEOUS
													|| block == YzCetiBlocks.YzCetiD.YZD_SEDIMENTARYROCK
													|| block == YzCetiBlocks.YzCetiD.YZD_METAMORPHIC) {
