...
public class BiomeYzCetiCBase extends BiomeSpace {
	...
	public BiomeYzCetiCBase(String singleName, BiomeProperties props) {
		...
		this.setStoneBlock(YzCetiBlocks.YzCetiC.YZC_METAMORPHIC);
		this.setPlanetForBiome(Planets.yzcetic);
	}

	public final void generateYzCetiCTerrain (World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 == 0) {
				...
				if (this.getBiomeType() == EnumBiomeType.OCEAN) {
					...
					else if (iblockstate2.getBlock() == BiomeYzCetiCBase.STONE) {
						if (j == -1) {
							if (k <= 0) {
								topState  = AIR;
								fillState = STONE;
