...
import net.romvoid95.common.world.cave.ConfigHolder.ConfigOption;
...
public class ConfigLoader {
	...
	/**
	 * Loads a config from file for a given planet.
	 * @param planet ExoPlanet
	 * @return ConfigHolder loaded from file for given dimension
	 */
	public static ConfigHolder loadConfigFromFileForDimension(World worldIn) {
		String fileName = "UndergroundConfigs.cfg";
		File file = null;
		try {
			if(worldIn.provider.getClass().getSuperclass().equals(WorldProviderWE_ExoPlanet.class)) {
				file = new File(((WorldProviderWE_ExoPlanet)worldIn.provider).getPlanetFolder() + fileName);
			} else {
				file = new File(((WorldProviderExoPlanet)worldIn.provider).getPlanetFolder() + fileName);
			}
		} catch (Exception e) {
			// TODO: handle exception
		}
		File configFile = file;
		ExoplanetsMod.logger.info(configFile.getPath());
