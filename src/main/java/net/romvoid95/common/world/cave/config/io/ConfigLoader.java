package net.romvoid95.common.world.cave.config.io;

import net.romvoid95.common.world.cave.config.util.ConfigHolder;
import net.romvoid95.common.world.cave.enums.*;
...
public class ConfigLoader {
     ...
     * The file must be located at {@code <configdirectory>/ExoplanetsMod-1_12_2/DIM<id>_config.cfg}
    ...
    public static ConfigHolder loadConfigFromFileForDimension(int dimensionID) {
        ...
        File configFile = new File(ExoplanetsMod.dir, fileName);

        if (!configFile.exists() || configFile.isDirectory()) {
            ExoplanetsMod.logger.info(String.format("Better Caves config file for dimension %d not found. Using global config...", dimensionID));
            return new ConfigHolder();
        }

        if (!configFile.canRead()) {
            ExoplanetsMod.logger.warn(String.format("Better Caves config file for dimension %d not readable. Using global config...", dimensionID));
            return new ConfigHolder();
        }

        ExoplanetsMod.logger.info(String.format("Reading Better Caves config from file for dimension %d...", dimensionID));
    ...
    private static ConfigHolder parseConfigFromFile(File file) {
        ...
        try {
                    ...
                    if (Character.isLetterOrDigit(line.charAt(i)) || ALLOWED_CHARS.indexOf(line.charAt(i)) != -1 || (quoted && line.charAt(i) != '"')) {
                        ...
                        switch (line.charAt(i)) {
                            ...
                            case '=':
                                ...
                                if (!isTypeSpecified) {
                                    ExoplanetsMod.logger.warn(String.format("Error in Better Caves config for %s (line %d): missing variable type specifier. Inferring String...", fileName, lineNum));
                                }
                                ...
                                if (target != null) {
                                    switch(type) {
                                        case INTEGER:
                                            if (!(target.type.equals(Integer.TYPE) || target.type.equals(Integer.class))) {
                                                ExoplanetsMod.logger.error(String.format("ERROR: WRONG TYPE for %s in config %s. Skipping...", fullName, fileName));
                                                i = line.length();
                                                continue;
                                            }
                                    ...
                                    ExoplanetsMod.logger.error(String.format("ERROR: INVALID PROPERTY %s in config %s. Skipping...", fullName, fileName));
                                    i = line.length();
                                    continue;
                                }

                                ExoplanetsMod.logger.debug(String.format("%s: overriding config option: %s", fileName, fullName));
            ...
            ExoplanetsMod.logger.error(String.format("ERROR LOADING BETTER CAVES CONFIG %s: %s.", fileName, e.toString()));
            ExoplanetsMod.logger.info("USING GLOBAL CONFIG FILE INSTEAD...");
            return new ConfigHolder();
        } finally {
