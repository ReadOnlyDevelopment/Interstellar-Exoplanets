...
package net.rom95.common.world.mapgen;
...
public class MapGenExoCaveGen extends MapGenBaseMeta {
	...
	public MapGenExoCaveGen(Block block, int surfaceMeta, int subSurfaceMeta, int stoneMeta) {
		this.block = block;
											...
											if (block == this.block) {
												if (metadata == this.subSurfaceMeta || metadata == this.stoneMeta) {
													primer.setBlockState(localX, localY, localZ,
														...
														.nextInt(MapGenExoCaveGen.BREAK_THROUGH_CHANCE) == 0) {
													primer.setBlockState(localX, localY, localZ,
															Blocks.AIR.getDefaultState());
												}
											}
