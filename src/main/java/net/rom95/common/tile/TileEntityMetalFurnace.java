...
import net.rom95.common.block.machine.container.ContainerMetalFurnace;
import net.rom95.common.lib.tile.TileSidedInventoryExo;
import net.rom95.common.utility.StackHelper;
...
public class TileEntityMetalFurnace extends TileSidedInventoryExo implements ITickable {
	...
	public List<String> getDebugLines() {
		...
		list.add("BONUS_NUGGETS_MIN = " + BONUS_DUST_MIN);
		list.add("BONUS_NUGGETS_MAX = " + BONUS_DUST_MAX);
		return list;
	}
	...
	public void setInventorySlotContents(int index, ItemStack stack) {
		...
		boolean flag = !current.isEmpty() && !stack.isEmpty() && stack.isItemEqual(current) && ItemStack.areItemStacksEqual(stack, current);
			...
			for (int outputId : OreDictionary.getOreIDs(outputPrimary)) {
				...
				if (inputName.startsWith("ore") && REGEX_ITEM_MATCH.matcher(outputName).find()) {
					...
					if (inputName.equals(outputName)) {
						// Same ore, can we find a dust?
						List<ItemStack> dust = StackHelper.getOres("dust" + inputName);
						if (!dust.isEmpty()) {
							ItemStack result = dust.get(0).copy();
							result.setCount(BONUS_DUST_MIN + ExoplanetsMod.random.nextInt(BONUS_DUST_MAX - BONUS_DUST_MIN + 1));
							return result;
						} else {
							return ItemStack.EMPTY;
						}
	...
	private boolean canSmelt() {
		...
		if (inputStack.isEmpty()) {
			...
			if (!outputSecondary.isEmpty()) {
				outputSecondary.setCount(BONUS_DUST_MAX);
			}
	...
	public void smeltItem() {
		if (canSmelt()) {
			...
			ItemStack input = getStackInSlot(SLOT_INPUT);
			ItemStack fuel = getStackInSlot(SLOT_FUEL);
			if (input.getItem() == Item.getItemFromBlock(Blocks.SPONGE) && input.getMetadata() == 1 && fuel.getItem() == Items.BUCKET) {
