package net.rom95.api.world.worldgen.feature;
...
public class ExoTreeFlatTop extends ExoTree {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		for (int a = 6 + rand.nextInt(3); a > -1; a--) {
			...
			while (sh < h) {
				this.placeLogBlock(world, new BlockPos(x + (int) (xd * c), y + sh, z + (int) (yd * c)), this.logBlock,
						this.generateFlag);
				sh++;
				c += 0.5f;
			}
			...
			for (i = -2; i <= 2; i++) {
				for (j = -2; j <= 2; j++) {
					if (Math.abs(i) + Math.abs(j) < 4) {
						this.placeLeavesBlock(world, new BlockPos(x + i, y + 1, z + j), this.leavesBlock,
								this.generateFlag);
					}
				}
			}
			...
			for (i = -3; i <= 3; i++) {
				for (j = -3; j <= 3; j++) {
					if (Math.abs(i) + Math.abs(j) < 5) {
						this.placeLeavesBlock(world, new BlockPos(x + i, y, z + j), this.leavesBlock,
								this.generateFlag);
					}
				}
			}
