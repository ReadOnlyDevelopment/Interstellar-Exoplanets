...
public class ExoTreeWillow extends ExoTree {
        ...
        for (int b = 0; b < branches; b++) {
            ...
            while (c < branchLenght) {
                ...
                this.placeLogBlock(world,
                    new BlockPos(x + (int) (c * xd), y + (int) hd, z + (int) (c * yd)), this.logBlock, this.generateFlag
                );
            }
            createLeavesAroundBranch(world, rand, x + (int) (c * xd), y + (int) hd, z + (int) (c * yd), 2, 1);
        }
    ...
    private void createTrunk(World world, Random rand, int x, int y, int z) {
        ...
        for (int t = 0; t < 5; t++) {
            sh = rand.nextInt(3) + y;
            while (sh > y - 3) {
                this.placeLogBlock(world, new BlockPos(x + pos[t * 2], sh, z + pos[t * 2 + 1]), this.logBlock, this.generateFlag);
                sh--;
            }
        }
    }
