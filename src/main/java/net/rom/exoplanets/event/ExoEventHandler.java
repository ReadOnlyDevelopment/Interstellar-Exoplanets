	...
	public void onPlanetDecorated(GCCoreEventPopulate.Post event) {
		if (event.world.provider instanceof WorldProviderMars) {
			genOre(event.world, event.pos,
					new WorldGenMinableMeta(ExoBlocks.OVERWORLD_ORE, 4, 0, true, MarsBlocks.marsBlock, 9), 6, 4, 18);
			genOre(event.world, event.pos,
					new WorldGenMinableMeta(ExoBlocks.OVERWORLD_ORE, 6, 1, true, MarsBlocks.marsBlock, 9), 10, 6, 30);
			genOre(event.world, event.pos,
					new WorldGenMinableMeta(ExoBlocks.OVERWORLD_ORE, 16, 2, true, MarsBlocks.marsBlock, 9), 15, 6, 70);

		}
		if (event.world.provider instanceof WorldProviderAsteroids) {

			genOre(event.world, event.pos,
					new WorldGenMinableMeta(ExoBlocks.OVERWORLD_ORE, 4, 0, true, AsteroidBlocks.blockBasic, 0), 6, 0, 255);
		}
	}
