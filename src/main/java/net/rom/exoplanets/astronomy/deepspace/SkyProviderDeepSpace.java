...
public class SkyProviderDeepSpace extends IRenderHandler {
	private static final ResourceLocation sunTexture = new ResourceLocation(Constants.ASSET_PREFIX,
	...
	public SkyProviderDeepSpace(ResourceLocation planet, boolean renderMoon, boolean renderSun) {
		this.sunSize = 17.5F / 1.2F / ExoUniverse.stationDeepSpace.getRelativeDistanceFromCenter().unScaledDistance;
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		GL11.glPushMatrix();
		float skyAngle = 15F / 16F * TransformerHooksClient
				.preViewRender((EntityLivingBase) ClientProxyCore.mc.getRenderViewEntity(), partialTicks, -1F);
		while (skyAngle < 0F)
			skyAngle += 360F;
		while (skyAngle > 360F)
			skyAngle -= 360F;
