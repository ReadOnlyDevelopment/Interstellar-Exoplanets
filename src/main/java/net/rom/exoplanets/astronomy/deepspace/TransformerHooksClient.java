...
import net.rom.exoplanets.astronomy.deepspace.blocks.BlockDeepStructure;
import net.rom.exoplanets.astronomy.deepspace.blocks.DeepSpaceBlocks;
...
public class TransformerHooksClient {
	...
	private static float ar = MathHelper.sqrt(a * a + b * b);
	...
	public static void preRenderChunk(RenderChunk chunk, BlockPos pos, double playerX, double playerY, double playerZ) {
		if (chunk.getWorld().provider instanceof WorldProviderDeepSpace) {
			int cz = pos.getZ() >> 4;
			int org = MathHelper.floor(playerZ) >> 4;
	...
	public static void edgeBlocks(BlockPos pos, IBlockState bs, ChunkCompileTaskGenerator generator, World world,
			CompiledChunk compiledchunk) {
		if (GalacticraftCore.isPlanetsLoaded && world.provider instanceof WorldProviderDeepSpace
				&& bs.getBlock().getDefaultState().getRenderType() != EnumBlockRenderType.INVISIBLE) {
			int z = pos.getZ() % 16;
			if (z < 0)
				z += 16;
			int py = pos.getY();
			if ((z == 0 || z == 15) && py < heightBaseline && py >= heightBaseline - 8) {
				BufferBuilder buffer = generator.getRegionRenderCacheBuilder()
						.getWorldRendererByLayerId(BlockRenderLayer.CUTOUT.ordinal());
				if (buffer != null) {
					Block wall;
					...
					brd.getBlockModelRenderer().renderModel(world, model, state, pos.add(0, 0, z == 0 ? -1 : 1), buffer,
							false);
				}
			}
		}
	...
	public static void preRenderEntity(Entity entity, double dX, double dY, double dZ, float partialTicks) {
		...
		if (entity.getEntityWorld() != null && entity.getEntityWorld().provider instanceof WorldProviderDeepSpace) {
			...
			double pZ = posZ - dZ;
			int cz = MathHelper.floor(posZ) >> 4;
			int org = MathHelper.floor(pZ) >> 4;
			...
			float z = (float) zz;
			int ccz = (MathHelper.floor(z) / 16) % 6;
			if (ccz < 0)
				ccz += 6;
			else
				ccz = (ccz + 1) % 6;
	...
	public static void adjust(int pz, int offsetZ, float h, double zz, float dX, float dY, float dZ) {
		...
		// Bring the chunk / entity to the origin with offset because it will be rotated
		// around pivot (0, ddY, 0)
		GL11.glTranslatef(dX, -ddY + dY, offsetZ * 16F + 8F + dZ);

		// To make a joined up circle, the angle needs to be slightly different
		// (thetaAdjust) for some chunks - org is the offset of that special chunk from
		// origin
		int org = pz % 6;
		if (org < 0)
			org += 6;
		...
		// (#1) Reverse the effect of thetaAdjust and org on the offsetZ == 0 chunk
		// (where the player is) - and apply the same to all chunks
		...
		// Rotate the chunk and move it around the circle, with adjustments for
		// thetaAdjust
		if (offsetZ > 0) {
		...
		// Finally, stabilise the offsetZ == 0 chunk by undoing the reversal at #1 above
		// (and again apply the same to all chunks)
		...
		// and also cancel the effect of the (dx, dy, dz) translation which the vanilla
		// chunk / entity renderer will now make
	...
	private static int getOffsetAdjust(int cz, int offsetZ) {
		...
		// 2 = 2 strut-circle interface angles clockwise (the segment quarter of the way
		// around)
		// etc
...
//        float ya = y;
		...
		if (y < 0F)
			GL11.glTranslatef(0.0F, 0.0F, y * z / 8F * reverse);
//        else
//        {
...
//        }
	...
	public static void preViewRender2(EntityLivingBase viewEntity, float partialTicks) {
		float y = (float) (viewEntity.lastTickPosY + (viewEntity.posY - viewEntity.lastTickPosY) * partialTicks)
				- heightBaseline;
...
//        }
	...
	public static AxisAlignedBB adjustEntityBB(Entity entity, float y, float z, AxisAlignedBB base) {
		int cz = (MathHelper.floor(z) / 16) % 6;
		cz = (cz + (z < 0F ? 6 : 1)) % 6;
		z = z % 16F;
		if (z < 0F)
			z += 16F;
