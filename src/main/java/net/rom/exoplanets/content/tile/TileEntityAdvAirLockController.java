...
public class TileEntityAdvAirLockController extends TileEntityAdvanced {
	...
	public TileEntityAdvAirLockController() {
		super("tile.adv_air_lock_seal.name");
	...
	public void update () {
		...
		if (!this.world.isRemote) {
			...
			if ((this.active || !this.redstoneActivation) && this.playerDistanceActivation) {
				...
				List<EntityPlayer> playersWithin  = this.world
				...
				for (x = this.protocol.minX + 1; x <= this.protocol.maxX - 1; x++) {
					for (z = this.protocol.minZ + 1; z <= this.protocol.maxZ - 1; z++) {
						...
						if (this.world.getBlockState(pos).getBlock()
								.isAir(this.world.getBlockState(pos), this.world, pos)) {
