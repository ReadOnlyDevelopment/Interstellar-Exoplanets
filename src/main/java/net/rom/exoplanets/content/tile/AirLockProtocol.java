...
class AirLockProtocol {
					...
					for (int y = -1; y <= 1; y++) {
						if (!(x == 0 && y == 0 && z == 0)) {
							...
							if (!this.checked.contains(testPos)) {
								...
								if (this.world.getTileEntity(testPos) instanceof TileEntityAdvAirLockController) {
	...
	public int calculate (boolean horizontal) {
		...
		if (this.airLocksDimension2Max == 0 || this.airLocksDimension2Min == 0
				|| (this.airLocksDimension1Min == 0 || this.airLocksDimension1Max == 0)
				|| this.airLocksDimension2Max != this.airLocksDimension2Min
				|| this.airLocksDimension1Max != this.airLocksDimension1Min) {
			return -1;
		}
	...
	private void checkDimensions () {
		for (int y = this.minY; y <= this.maxY; y++) {
			...
			if (tileAt instanceof TileEntityAdvAirLockController) {
