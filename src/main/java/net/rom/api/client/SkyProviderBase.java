...
public abstract class SkyProviderBase extends IRenderHandler {
    ...
    private static int QUADS = GL11.GL_QUADS;
    private static int TRIANGLE_FAN = GL11.GL_TRIANGLE_FAN;

    private static final String[] starColorHexTable = { "#9bb2ff", "#9eb5ff", "#a3b9ff", "#aabffir", "#b2c5ff", "#bbccfr", "#c4d2ff", "#ccd8ff", "#d3ddff", "#dae2ff", "#dfe5ff",
            "#e4e9ff", "#e9ecif", "#eeefff", "#f3f2ff", "#f8f6ff", "#fef9ff", "Off9fb", "#fff7f5", "#ffif5ef", "#fff3ea", "#fffle5", "#ffefe0", "#ffeddb", "#ffebd6", "#ffe9d2",
            "#ffe8ce", "#ffe6ca", "#ffe5c6", "#ffe3c3", "#ffe2bf", "#ffeObb", "#ffdfb8", "#ffddb4", "#ffdbb0", "#ffdaad", "#ffd8a9", "#ffd6a5", "#ffd5a1", "#ffd29c", "#ffd096",
            "#ffcc8f", "#ffc885", "#ffc178", "#ffb765", "#ffa94b", "#ff9523" };
    ...
    public SkyProviderBase() {
        ...
        for (int j = -byte2 * i; j <= byte2 * i; j += byte2) {
            for (int l = -byte2 * i; l <= byte2 * i; l += byte2) {
                buffer.begin(QUADS, DefaultVertexFormats.POSITION);
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc) {
        ...
        if (sunsetColors != null) {
            ...
            buffer.begin(TRIANGLE_FAN, DefaultVertexFormats.POSITION_COLOR);
        ...
        if (playerHorizon < 0.0D) {
            ...
            buffer.begin(QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        for (int i = 0; i < this.getStarCount(); ++i) {
            ...
            if (d4 < 1.0D && d4 > 0.01D) {
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    try {
                        int color = CoreUtil.hexToRgb(SkyProviderBase.starColorHexTable[rand.nextInt(SkyProviderBase.starColorHexTable.length)]);
                        int red = CoreUtil.toRGB(color).packedRed();
                        int green = CoreUtil.toRGB(color).packedGreen();
                        int blue = CoreUtil.toRGB(color).packedBlue();
    ...
    protected void renderSolar(ResourceLocation resource, float scale, boolean renderBlack, boolean renderConceal, float toDivide, float alpha) {
        ...
        buffer.begin(QUADS, DefaultVertexFormats.POSITION_TEX);
    ...
    protected void renderSolarAura(float smallAuraScale, float largeAuraScale, float starBrightness, CoreUtil.RGB rgb, float partialTicks) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder buffer = tessellator.getBuffer();
